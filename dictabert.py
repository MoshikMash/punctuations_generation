...
def download_s3_dir(path, out_path, keys):
    s3_client = boto3.client('s3', aws_access_key_id='AKIAUPGFS4H6DLNXX5VI', aws_secret_access_key='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', config=Config(region_name='us-east-1'))
    ...
    for key in keys:
        ...
        s3_key = f'{path}/{key}'
        file_len = s3_client.get_object_attributes(Bucket='dictabertmodels',Key=s3_key, ObjectAttributes=['ObjectSize'])["ObjectSize"]
        s3_client.download_file('dictabertmodels', f'{path}/{key}', local_path, Callback=ProgressPercentage(key, file_len)) # save to same path
...
def get_final_path(path):
    ...
    if path and path[0] not in './~':
        ...
        try: 
            ...
            download_s3_dir(path.replace('\\', '/'), tmp_out_path, ['description.txt', 'config.json', 'vocab.txt', 'pytorch_model.bin'])
